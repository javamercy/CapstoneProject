...
struct VehicleState {
    ...
    VehicleState(double setX, double setY, double setPsi, double setV): x(setX), y(setY), psi(setPsi), V(setV),
                                                                        yaw_rate(0.0), steering(0.0) {
    }

    VehicleState(double setX, double setY, double setPsi, double setV, double setPsiDot, double setSteering): x(setX),
        y(setY), psi(setPsi), V(setV), yaw_rate(setPsiDot), steering(setSteering) {
    }
};
...
public:
    ...
    virtual bool update(double time, double dt, VehicleState state) {
        return false;
    }

    virtual double getVelocityCommand() {
        return m_velocity_command;
    }

    virtual double getSteeringCommand() {
        return m_steering_command;
    }
...
public:
    MotionCommandStraight(double command_time, double command_velocity): m_command_time(command_time),
...
public:
    MotionCommandTurnTo(double command_heading, double command_velocity): m_command_heading(command_heading),
                                                                          m_command_velocity(command_velocity) {
    }
...
public:
    MotionCommandMoveTo(double command_x, double command_y, double command_velocity): m_command_x(command_x),
        m_command_y(command_y), m_command_velocity(command_velocity) {
    }
...
class BicycleMotion {
public:
    BicycleMotion(): m_initial_state(VehicleState(0, 0, 0, 0)), m_wheel_base(4.0), m_max_velocity(28.0),
                     m_max_acceleration(2.0), m_max_steering(0.8) {
        reset();
    }

    BicycleMotion(double x0, double y0, double psi0, double V0): m_initial_state(VehicleState(x0, y0, psi0, V0)),
                                                                 m_wheel_base(4.0), m_max_velocity(28.0),
                                                                 m_max_acceleration(2.0), m_max_steering(0.8) {
        reset();
    }
    ...
    void setSteeringCmd(double steer) {
        m_steering_command = steer;
    }

    void setVelocityCmd(double accel) {
        m_velocity_command = accel;
    }
...
public:
    ...
    VehicleState getVehicleState() const {
        return m_vehicle_model.getVehicleState();
    }
    ...
    void render(Display &disp) {
        ...
        disp.drawLines(transformPoints(transformPoints(m_wheel_lines, m_wheel_fl_offset, steeringPsi), carPosOffset,
                                       carPsiOffset));
        disp.drawLines(transformPoints(transformPoints(m_wheel_lines, m_wheel_fr_offset, steeringPsi), carPosOffset,
                                       carPsiOffset));
