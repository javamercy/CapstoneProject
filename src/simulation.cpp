...
void Simulation::reset() {
    ...
    m_gps_sensor.setGPSDeniedZone(m_sim_parameters.gps_denied_x, m_sim_parameters.gps_denied_y,
                                  m_sim_parameters.gps_denied_range);
    ...
    m_car.reset(m_sim_parameters.car_initial_x, m_sim_parameters.car_initial_y, m_sim_parameters.car_initial_psi,
                m_sim_parameters.car_initial_velocity);
        ...
        for (unsigned i = 0; i < m_time_multiplier; ++i) {
            ...
            if (m_sim_parameters.gps_enabled) {
                if (m_time_till_gps_measurement <= 0) {
                    GPSMeasurement gps_meas = m_gps_sensor.generateGPSMeasurement(
            ...
            if (m_sim_parameters.lidar_enabled) {
                if (m_time_till_lidar_measurement <= 0) {
                    std::vector<LidarMeasurement> lidar_measurements = m_lidar_sensor.generateLidarMeasurements(
                        m_car.getVehicleState().x, m_car.getVehicleState().y, m_car.getVehicleState().psi, m_beacons);
...
void Simulation::render(Display &disp) {
    ...
    disp.setView(m_view_size * disp.getScreenAspectRatio(), m_view_size, m_car.getVehicleState().x,
                 m_car.getVehicleState().y);
    ...
    if (m_sim_parameters.gps_denied_range > 0) {
        std::vector<Vector2> zone_lines = generateCircle(m_sim_parameters.gps_denied_x, m_sim_parameters.gps_denied_y,
                                                         m_sim_parameters.gps_denied_range);
    ...
    std::string gps_string = string_format("GPS: %s (%0.1f Hz)", (m_sim_parameters.gps_enabled ? "ON" : "OFF"),
                                           m_sim_parameters.gps_update_rate);
    std::string lidar_string = string_format("LIDAR: %s (%0.1f Hz)", (m_sim_parameters.lidar_enabled ? "ON" : "OFF"),
                                             m_sim_parameters.lidar_update_rate);
    std::string gyro_string = string_format("GYRO: %s (%0.1f Hz)", (m_sim_parameters.gyro_enabled ? "ON" : "OFF"),
    ...
    std::string kf_yaw_string = string_format("   Heading: %0.2f deg",
                                              m_kalman_filter.getVehicleState().psi * 180.0 / M_PI);
    ...
    disp.drawText_MainFont("Speed Multiplier (+/-) Key: [ / ] ", Vector2(x_offset, y_offset + stride * 2), 1.0,
                           {255, 255, 255});
    disp.drawText_MainFont("Zoom (+/-) Key: + / - (keypad)", Vector2(x_offset, y_offset + stride * 3), 1.0,
                           {255, 255, 255});
    disp.drawText_MainFont("Motion Profile Key: 1 - 9,0", Vector2(x_offset, y_offset + stride * 4), 1.0,
                           {255, 255, 255});
