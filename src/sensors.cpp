...
// GPS Sensor
GPSSensor::GPSSensor(): m_rand_gen(std::mt19937()), m_noise_std(0.0), m_error_prob(0.0), m_gps_denied_x(0.0),
                        m_gps_denied_y(0.0), m_gps_denied_range(-1.0) {
}
...
void GPSSensor::setGPSNoiseStd(double std) {
    m_noise_std = std;
}

void GPSSensor::setGPSErrorProb(double prob) {
    m_error_prob = prob;
}
...
// Lidar Sensor
LidarSensor::LidarSensor(): m_rand_gen(std::mt19937()), m_range_noise_std(0.0), m_theta_noise_std(0.0),
                            m_max_range(90.0), m_id_enabled(true) {
}
...
void LidarSensor::setLidarMaxRange(double range) {
    m_max_range = range;
}

void LidarSensor::setLidarDAEnabled(bool id_enabled) {
    m_id_enabled = id_enabled;
}

std::vector<LidarMeasurement> LidarSensor::generateLidarMeasurements(double sensor_x, double sensor_y,
                                                                     double sensor_yaw, const BeaconMap &map) {
